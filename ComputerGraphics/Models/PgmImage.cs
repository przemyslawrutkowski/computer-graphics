    ...
    public class PgmImage : ImageBase
    {
                ...
                for (int i = 0; i < Height; i++)
                {
                    for (int j = 0; j < Width; j++)
                    {
                        if (fs.Position >= fs.Length)
                            throw new EndOfStreamException($"Unexpected end of file at pixel ({i}, {j}).");

                        Pixels[i, j] = MaxVal < 256 ? br.ReadByte() : ReadBigEndianUShort(br);
                    }
                }
            }
            else
            {
                ...
                for (int i = 0; i < Height; i++)
                {
                    for (int j = 0; j < Width; j++)
                    {
                        ...
                        Pixels[i, j] = ushort.Parse(pixelToken);
                    }
                }
            }
        }
        ...
        public override void Save(string filePath, bool isBinary)
        {
            if (isBinary)
            {
                ...
                bw.Write(Encoding.ASCII.GetBytes($"P5\n{Width} {Height}\n{MaxVal}\n"));
                ...
                for (int i = 0; i < Height; i++)
                {
                    ...
                    {
                        if (MaxVal < 256)
                            bw.Write((byte)Pixels[i, j]);
                        else
                            WriteBigEndianUShort(bw, Pixels[i, j]);
                    }
                }
            }
            else
